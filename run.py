...
def choose_level():
	...
	while(level != "1" and level != "2" and level != "3"):
		...
		level = int(input("What the level wanted? [Input 1 to Easy Made; Input 2 to Hard Made; Input 3 to Pro Made; 4 to Exit Execution]: "))
		if(level == "4"):
			sys.exit()
	if(level == "1"):
		return 4
...
def canot_positing(row, column, BUILD_BOARD, LIMIT_BOARD):
	check = []
	checks = [[row-1 ,row-1, column, 0], [column-1, row, column-1, 0], [row+1, row+1, column, LIMIT_BOARD], [column+1, row, column+1, LIMIT_BOARD]]
...
def summon_abyss_and_monster_and_gold(BUILD_BOARD, LIMIT_BOARD, loops):
	for i in range(loops+2):
		...
		if(i != loops+1):
			while((row == LIMIT_BOARD - 1  and column == 0) or (row == LIMIT_BOARD and column == 1) or (BUILD_BOARD[row][column] == PLAYER) or (BUILD_BOARD[row][column] == ABYSS) or (BUILD_BOARD[row][column] == MONSTER)):
				row = random.randint(0, LIMIT_BOARD)
				column = random.randint(0, LIMIT_BOARD)
			if i == loops:
			...
			while((BUILD_BOARD[row][column] == PLAYER) or (BUILD_BOARD[row][column] == ABYSS) or canot_positing(row, column, BUILD_BOARD, LIMIT_BOARD)):
				row = random.randint(0, LIMIT_BOARD)
				column = random.randint(0, LIMIT_BOARD)
...
def summom_stenchs_and_breezes(BUILD_BOARD, LIMIT_BOARD, SIZE_BOARD):
	...
	for i in positions:
		...
		for i in checks:
			if(can_put(i[0], i[1], LIMIT_BOARD) and BUILD_BOARD[i[0]][i[1]] != ABYSS and valor not in BUILD_BOARD[i[0]][i[1]]):
				BUILD_BOARD[i[0]][i[1]] += valor

def remapping_keys():
	clear_display()
	commands = ["w", "s", "a", "d", "u", "j", "h", "k"]
	descritiones = ["move up", "move down", "move left", "move right", "shoot up", "shoot down", "shoot left", "shoot right"]
			...
			if(check_wall(row_player, column_player, LIMIT_BOARD, command, commands)):
				...
				if(GOLD in BUILD_BOARD[row_player][column_player]):
					if(pick_gold(row_player, column_player, BUILD_BOARD, RUN_BOARD, SIZE_BOARD, scores)):
						play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/withgold.mp3', "gold")
						scores += 1000

				if(game_over(row_player, column_player, BUILD_BOARD, SIZE_BOARD) == "abyss" or game_over(row_player, column_player, BUILD_BOARD, SIZE_BOARD) == "monster"):
					if(game_over(row_player, column_player, BUILD_BOARD, SIZE_BOARD) == "monster"):
						play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/withmonster.mp3', "monster")
					else:
						play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/withabyss.mp3', "abyss")
					scores -= 10000
				...
				if(bool(sensations[0]) == True):
					if(len(sensations) > 1):
						print(sensations[1])
						if(sensations[1] == "The player is facing stenches."):
							play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/withstench.mp3', "stench")		
						elif(sensations[1] == "The player is facing breezes."):
							play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/withbreeze.mp3', "breeze")
						time.sleep(1.0)
		elif(command == commands[4] or command == commands[5] or command == commands[6] or command == commands[7]):
			play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/hit.mp3', "hit")
			repositioning_player(row_player, column_player, BUILD_BOARD, SIZE_BOARD)
			...
			for i in range(SIZE_BOARD):
				for j in range(SIZE_BOARD):
					if("x" in BUILD_BOARD[i][j]):
						...
						print("")
						show_top5(level)
						play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/monsterdead.mp3', "kill")
						sys.exit()
			repositioning_player(row_player, column_player, BUILD_BOARD, SIZE_BOARD)
			show_board(BUILD_BOARD, SIZE_BOARD, scores)
			print("His arrow did not hit the monster. You lost!")
			play_sound('D:/Documentos/Computação/Fundamentos da Programação/Jogo/Jogo Pedro/'+path_sound+'/monsteralive.mp3', "not_hit")
			sys.exit()
...
def kill_monster(row_player, column_player, BUILD_BOARD, RUN_BOARD, SIZE_BOARD, command, commands, scores):
	checks = [[commands[4], row_player, -1, "row_player"], [commands[5], row_player, SIZE_BOARD, "row_player"], [commands[6], column_player, -1, "column_player"], [commands[7], column_player, SIZE_BOARD, "column_player"]]
	for i in checks:
		if(command in i):
			index1 = 0
			index2 = 0
...
def check_wall(row_player, column_player, LIMIT_BOARD, command, commands):
	return (row_player != 0 and command == commands[0]) or (row_player != LIMIT_BOARD and command == commands[1]) or (column_player != 0 and command == commands[2]) or (column_player != LIMIT_BOARD and command == commands[3])
